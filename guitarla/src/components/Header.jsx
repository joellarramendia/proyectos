...
function Header ({cart, removeFromCart, increaseQuantity, decreaseQuantity, clearCart}) {
  ...
  //state derivado
  const isEmpty = useMemo(() => cart.length === 0, [cart]) //Verificar si el carrito está vacío
  const cartTotal = useMemo( () => cart.reduce((total, item) => total + (item.price * item.quantity), 0), [cart]) //Calcular total a pagar
